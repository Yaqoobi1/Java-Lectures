...
public class SDETClass {
	// creating a list to store students
	 	...
	 	public SDETClass() {
	 		...
	 		listOfStudent = new Student[50]; // we initialize the variable list we have in this class to default size of 50 that we defined
	 	}
	 	public SDETClass(int size) {
	 		// we are in the constructor, so we should initialize our variables
	 		
	 		listOfStudent = new Student[size]; // so we initialize the array that we have to the size that is provided to us in the arg of this method
	 	}
